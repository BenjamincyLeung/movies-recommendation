...
def get_movie_recommendation(id):
    ...
    movies = postgresql_to_dataframe(
        conn, "select films.id, name, image from films", ["film_id", "title", "image"])
    ...
    # Preparing data for NearestNeighbors algorithm of films
    ...
    # Uncomment the below filtering logic if each films having number of ratings greater than 4
    # final_dataset = final_dataset.loc[no_user_voted[no_user_voted > 4].index, :]
    ...
    final_dataset = final_dataset.loc[:,
                                      no_movie_voted[no_movie_voted > 50].index]
